...
The differences between these DataFrames is subtle. In the second one, `year` has been moved to a column (where the `section` is the index label&mdash;note that each element has a section listed on its row). This would allow you to do further analysis on the data (which is now organized only by section, with year as a feature rather than an index).
