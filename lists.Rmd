...
The `'a'` (the first element) is at _index_ 0, `'e'` (the second element) is at index 1, and so on. This also means that the last element can be found at the index `length_of_list - 1`. This pattern is why values like `range(5)` _include_ `0` but _exclude_ the last `5`.
